#!/usr/bin/env python3
"""
CLI entry-point for listing open pull requests.
"""
import sys
...
@click.option("--state", type=click.Choice(["open","closed","all"], case_sensitive=False),
              default="open", show_default=True, help="PR state filter.")
@click.option("--debug/--no-debug", default=False, help="Enable debug logging.")
def main(owner, repo, state, debug):
    if debug:
    ...
    lister = PullRequestLister(client, state)
    try:
        lister.list()
